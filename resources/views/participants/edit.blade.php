...
@section('mainContent')
    <div class="row">
        <div class="col-md-2"></div>
            <div class="col-md-8">
                <form class="form-horizontal" method="POST" action="{{ route('participants.update')}}">
                
                    @csrf  {{-- tak nak bagi org lain isi form--}}
                    <input type="hidden" name="participant_id" value="{{$participantM->id}}">
                    <input type="hidden" name="event_id" value="{{$participantM->event_id}}">
